...
| `BP_GameXiShu_GuanLiQi` | `/Game/Blueprints/ZiYuanGuanLi/BP_GameXiShu_GuanLiQi` | manager holding default-object maps for game-coefficient and recruitment-cap keys; variants `_Management`, `_Action`, `_Creative`, `_PVP`, one per `ECustomGameMode` member selected by `BP_CustomGameModeManager.GameXiShuGuanLiQiClassMap` | asset-level verified |
