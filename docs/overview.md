...
| Talent pools and tiers | [talents.md](game-reference/talents.md) | `DT_GiftZhengMian`, `DT_GiftZhengMian_Custom`, `DT_GiftFuMiann`, `DT_GiftZongBiao` `BaseWeight`; `BP_ManRenRandomConfig` (`BornChuShenCiTiaoList`, `BornJingLiCiTiaoMap`, `BornBuLuoCiTiaoMap`); `DT_PinZhiGoodNGStarWeight`, `DT_PinZhiGoodNGAddPr` | Replace pool memberships, pin origin and experience gates, reweight effect rows, and force star tiers through the quality star weights | Yes | High membership, medium weights |
...
| Preferences | [talents.md](game-reference/talents.md) | `DT_GiftXiHaoBiao`, `DT_GiftZongBiao` `XiHao`-source rows | Remove the stat-bearing gear preference rows and keep the mood-only like/aversion rows | Partial: rows yes, the native count roll of 0–3 is not data-editable | Medium |
| Roster ramp | [roster-limits.md](game-reference/roster-limits.md) | `BP_Mask_XiuFu01_1012` (`AddZhaoMuNum` tiers), `BP_GameXiShu_GuanLiQi` default-object maps; `GameXishu.json` keys `GeRenMaxZhaoMuCount`, `_Two`, `_Three`, `GongHuiMaxZhaoMuCount`, `ManRenChuZhanCount` | Rebuild the mask node with 15 `AddZhaoMuNum` tiers gated every four awareness levels and add the matching keys to both manager maps | Yes | In-game verified (3 → 48) |
...
- Grow the personal roster cap through the 15-tier mask ramp, verified in game from native base 3 to 48.
...
- Clearing the Shifting Sands archetype origin gift: `DT_CustomizeNPC_Egypt.CustomizeProfMaxLv` is cleared, but the `CustomizeNGMap` origin gift is not.
...
`mod-status.md` tracks this by separating three verification levels: **in-game verified** (the cap default-object constants and the roster ramp), **asset-level verified** (re-parse and byte-identical round-trip only), and **pending in-game verification** (mastery draws, added class and pool rows, the frozen class/base boundary, the DLC caps, and the localization override priority).
